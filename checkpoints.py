    ...
    def longestdistance(self, checkpoints):
        ...
        while len(checkpoints) != 0:
            ...
            for i in checkpoints:
                if i < temp_lowest:
                    temp_lowest = i
            sorted_checkpoints.append(temp_lowest)
            checkpoints.remove(temp_lowest)

        longest_distance = 0
        for i in range(len(sorted_checkpoints)-1):
            if sorted_checkpoints[i+1] - sorted_checkpoints[i] > longest_distance:
                longest_distance = sorted_checkpoints[i+1] - sorted_checkpoints[i]
